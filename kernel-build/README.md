...
#### Build determinism — how far it goes

`build.sh` pins `KBUILD_BUILD_USER`, `KBUILD_BUILD_HOST` and
`KBUILD_BUILD_TIMESTAMP`, so the banner no longer carries the build host or
wall-clock time. That removed most of the run-to-run variance — measured, two CI
builds of one commit went from ~40 KB apart to 6 bytes.

**Builds are still not bit-identical, and this is expected.** Measured across two
CI runs of the same commit: 1,088 differing bytes out of 30.8 MB (0.0035%), in 11
regions, all inside a 1320-byte DER SEQUENCE. The kernel contains the string
`Build time autogenerated kernel key`: with `CONFIG_MODULE_SIG` on and no
`certs/signing_key.pem` present, the build generates a fresh RSA key and
self-signed certificate every time and embeds the public cert. The differing
regions are that cert — 20-byte SHA-1 key identifiers, the DER body, and the
ASCII serial/validity digits. On arm64 the new cert also compresses differently,
which is the 6-byte size delta; on x86_64 the payload is uncompressed so the size
is unchanged.

So two builds of one commit differ **only** in the module-signing cert. That is
enough to compare a CI kernel against a known-good one — same version banner,
same size to within a few bytes, differences confined to the certs region — but
it is not a hash match, so do not write a check that expects one.

Making it bit-identical means pinning the signing key, which means storing a
private key somewhere. Do not commit one: the pattern to follow is the keybox —
Secret Manager, injected at build time — not a file in the repo. Disabling
`CONFIG_MODULE_SIG` would also do it, but that is a config change to the kernel
